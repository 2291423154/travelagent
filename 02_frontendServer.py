...
# 检查用户会话状态并尝试恢复
def check_and_restore_session(user_id: str, session_id: str, task_id: str = None):
    """
    检查用户会话状态并尝试恢复
    ...
    参数:
        user_id: 用户ID
        session_id: 会话ID
        task_id: 任务ID（可选，默认使用全局 task_id）
        ...
        if status_response["status"] == "interrupted":
            ...
            with Progress() as progress:
                ...
                for i in range(max_attempts):
                    attempt_count = i
                    # 检查状态
                    current_status = get_agent_status(user_id, session_id, get_current_task_id())
                ...
                # 获取最新状态（递归调用）
                return check_and_restore_session(user_id, session_id, get_current_task_id())
...
# 处理工具使用审批类型的中断
def handle_tool_interrupt(interrupt_data, user_id, session_id, task_id=None):
    """
    处理工具使用审批类型的中断
    ...
    try:
        while True:
            if user_input.lower() == "yes":
                response = resume_agent(user_id, session_id, get_current_task_id(), "accept")
                break
            elif user_input.lower() == "no":
                response = resume_agent(user_id, session_id, get_current_task_id(), "reject")
                break
            elif user_input.lower() == "edit":
                # 获取新的查询内容
                new_query = Prompt.ask("[highlight]请调整新的参数[/highlight]")
                response = resume_agent(user_id, session_id, get_current_task_id(), "edit", args={"args": json.loads(new_query)})
                break
            elif user_input.lower() == "response":
                # 获取新的查询内容
                new_query = Prompt.ask("[highlight]不调用工具直接反馈信息[/highlight]")
                response = resume_agent(user_id, session_id, get_current_task_id(), "response", args={"args": new_query})
                break
            else:
...
def process_agent_response(response, user_id):
    ...
    try:
        ...
        if status == "interrupted":
            ...
            try:
                # 进入中断处理函数
                return handle_tool_interrupt(interrupt_data, user_id, session_id, get_current_task_id())
...
def main():
    ...
    # 检查会话是否存在并尝试自动恢复现有会话
    has_active_session, session_status = check_and_restore_session(user_id, session_id, get_current_task_id())
    ...
    while True:
        try:
            ...
            # 处理特殊命令 获取指定用户当前会话的状态数据
                ...
                # 获取指定用户当前会话的状态数据
                status_response = get_agent_status(user_id, session_id, get_current_task_id())
                console.print(Panel(
            ...
            # 获取指定用户当前会话的状态数据
            latest_status = get_agent_status(user_id, session_id, get_current_task_id())
